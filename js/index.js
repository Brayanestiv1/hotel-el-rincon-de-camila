// cargar el api en el localstorage al inciar la pagina

document.addEventListener('DOMContentLoaded', () => {
    const API_URL = 'http://localhost:3000'; // URL de tu JSON Server
    const STORAGE_KEY = 'apiData'; // Clave para guardar los datos en localStorage
  
    // Función para cargar los datos desde JSON Server
    const fetchData = async () => {
      try {
        ...
        renderData(data); // Renderiza los datos en la página
      } catch (error) {
    ...
    const init = () => {
      ...
      if (storedData) {
        console.log('Cargando datos desde localStorage:', JSON.parse(storedData));
        renderData(JSON.parse(storedData)); // Renderiza los datos almacenados
      } else {
        console.log('Datos no encontrados en localStorage, cargando desde JSON Server...');
        fetchData(); // Carga los datos desde el servidor
      }
    };
    ...
    const renderData = (data) => {
      ...
      data.forEach(item => {
        ...
        div.textContent = `ID: ${item.id}, Nombre: ${item.nombre}, Precio: ${item.precio}`;
